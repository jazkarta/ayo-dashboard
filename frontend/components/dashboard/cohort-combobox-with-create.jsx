...
export default function CohortComboboxWithCreate({ value, onChange, disabled = false }) {
  ...
  useEffect(() => {
    if (!open) return;
    const timer = setTimeout(() => fetchCohorts(search), 300);
  ...
  const showCreate = trimmedSearch.length > 0 && !hasExactMatch && !loading;
      ...
      <PopoverTrigger asChild>
        ...
        >
          <span className={selectedCohort ? "text-foreground" : "text-muted-foreground"}>
            {selectedCohort ? selectedCohort.name : "Select or create a cohort"}
          </span>
